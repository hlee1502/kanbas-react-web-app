...
function WorkingWithArrays() {
    ...
    return(
        <div>
            ...
            <textarea className="form-control mb-2" onChange={(e) => setTodo({...todo, description: e.target.value})} value={todo.description} type="text"/> 

            <input className="form-control mb-2" onChange={(e) => setTodo({...todo, due: e.target.value})} value={todo.due} type="date"/>

            <label>
                <input className="me-2" onChange={(e) => setTodo({...todo, completed: e.target.checked})} value={todo.completed} type="checkbox"/>Completed
            </label>

            <button className="btn btn-warning mb-2 w-100" onClick={postTodo}>Post Todo</button>
            
            <button className="btn btn-info mb-2 w-100" onClick={updateTodo}>Update Todo</button>
            ...
            <ul className="list-group">{todos.map((todo) => (
                <li key={todo.id} className="list-group-item">
                    <button onClick={() => fetchTodoById(todo.id)} className="btn btn-warning me-2 float-end">Edit</button>
                    {/* <button onClick={() => removeTodo(todo)} className="btn btn-danger float-end me-2">Remove</button> */}
                    <button onClick={() => deleteTodo(todo)} className="btn btn-danger float-end me-2">Delete</button>
                    <input className="me-2" checked={todo.completed} type="checkbox" readOnly/>{todo.title}
            ...
            <a href={`${API}/${id}`} className="btn btn-primary me-2">Get Todo by ID</a>
